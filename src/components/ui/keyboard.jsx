...
const getKeyInputValue = (keyCode) => {
  ...
  if (Object.prototype.hasOwnProperty.call(KEY_INPUT_VALUES, keyCode)) {
    return KEY_INPUT_VALUES[keyCode];
  }
...
const KeyboardProvider = ({
  ...
  const emitKey = useCallback(
    (keyCode) => {
      ...
      if (value !== null) onKeyPress?.(value);
    },
    [onKeyPress],
  );
...
export const Keyboard = ({
  ...
  return (
    <KeyboardProvider
      capturePhysicalKeys={capturePhysicalKeys}
...
const Key = ({
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      <button
        type="button"
        aria-label={keyCode ? getKeyDisplayLabel(keyCode) : undefined}
        tabIndex={-1}
